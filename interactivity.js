...
(function(){
  ...
  /* ───── LIVE TOOL DATA CACHE ─────
     Populated by async fetch on load; read synchronously by knownAlways.
     Refreshed when the user saves data in a tool modal via window event.
  */
  const _toolCache = { health: null, notion: null, strava: null, diary: null, spotify: null };

  function _getProfileId() {
    try {
      const p = JSON.parse(sessionStorage.getItem('kairos_profile') || 'null');
      if (p && p.id) return p.id;
    } catch {}
    return localStorage.getItem('kairos_profile_id') || 'demo';
  }

  async function _refreshToolCache() {
    const pid = _getProfileId();
    const API = window.KAIROS_API_URL || 'http://localhost:8010';
    try {
      const [hRes, nRes, sRes] = await Promise.allSettled([
        fetch(`${API}/health-data/get?profile_id=${encodeURIComponent(pid)}`).then(r => r.json()),
        fetch(`${API}/notion/tasks?profile_id=${encodeURIComponent(pid)}`).then(r => r.json()),
        fetch(`${API}/strava/activity?profile_id=${encodeURIComponent(pid)}`).then(r => r.json()),
      ]);
      if (hRes.status === 'fulfilled') _toolCache.health  = hRes.value.record || null;
      if (nRes.status === 'fulfilled') _toolCache.notion  = nRes.value || null;
      if (sRes.status === 'fulfilled') _toolCache.strava  = sRes.value.activity || null;
    } catch {}
  }

  // Refresh on load + when a tool saves data
  _refreshToolCache();
  window.addEventListener('kairos:tool-saved', _refreshToolCache);
  ...
  */
  const AGENTS = {
    ...
    sueno: {
      ...
      knownAlways: () => {
        const h = _toolCache.health;
        if (h && h.sleep_hours) return [`Horas anoche · ${h.sleep_hours}h`, ...(h.heart_rate ? [`FC reposo · ${h.heart_rate} bpm`] : [])];
        return ['Horas anoche · 7.4h', 'Despertar · 7:12'];
      },
      questions: [
        {
          key: 'sueno:quality',
    ...
    foco: {
      ...
      knownAlways: () => {
        const n = _toolCache.notion;
        if (n && n.tasks && n.tasks.length) {
          const pending = n.tasks.filter(t => !t.done).length;
          const done = n.tasks.filter(t => t.done).length;
          return [`Tareas pendientes · ${pending}`, `Completadas hoy · ${done}`];
        }
        return ['Sesiones esta semana · 3', 'Récord · 95 min ininterrumpidos'];
      },
      questions: [
        {
          key: 'foco:priority',
    ...
    energia: {
      ...
      knownAlways: () => {
        const h = _toolCache.health;
        const s = _toolCache.strava;
        const facts = [];
        if (h && h.steps)       facts.push(`Pasos hoy · ${h.steps.toLocaleString()}`);
        if (s && s.sport)       facts.push(`Actividad · ${s.sport} ${s.duration_min} min`);
        if (!facts.length)      facts.push('Pasos hoy · 4.2k', 'Hidratación · 1.2L');
        return facts;
      },
      questions: [
        {
